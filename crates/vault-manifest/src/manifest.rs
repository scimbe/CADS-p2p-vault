        ...
        match self.entries.get(&path) {
            ...
            Some(existing) => {
                if incoming.hlc > existing.hlc {
                    ...
                    // blob_hash keeps the "primary" path stable across replicas;
                    // the loser survives under a conflict-suffixed path.
                    let (winner, loser) = if incoming.blob_hash >= existing.blob_hash {
...
mod tests {
    ...
    fn later_hlc_wins_on_the_same_path() {
        ...
        assert_eq!(m.get("a.txt").unwrap().blob_hash, "h2");
        assert_eq!(m.len(), 1);
    }
    ...
    fn merge_converges_regardless_of_order() {
        ...
        assert_eq!(left_then_right.get("a.txt").unwrap().blob_hash, "h3");
        assert_eq!(right_then_left.get("a.txt").unwrap().blob_hash, "h3");
